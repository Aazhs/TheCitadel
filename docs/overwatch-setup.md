# Overwatch System Setup Guide

This guide covers setting up the **Overwatch Cog** — a personal overwatch system that creates a private channel on your server where the bot pings you on a 15-minute loop, demands progress updates, and escalates if you go dark. It uses a local Ollama LLM and a local SQLite database.
...
The overwatch cog uses plain text messages (e.g. `!start auth module 45`) in the private channel, which requires the **Message Content Intent**.
...
> [!IMPORTANT]
> This is additive — enabling Message Content Intent does **not** change or break any existing slash command behavior. The cog only reads text in the private `#overwatch-zone` channel.
...
The bot will create a channel called `#overwatch-zone` with permission overwrites:
...
```
Message Content Intent enabled (overwatch cog)
Local SQLite database initialised at ./overwatch.db
Loaded overwatch cog (user: 123456789012345678, guild: 987654321098765432)
```

The first time the check-in loop runs, it will automatically create the `#overwatch-zone` channel in your server. You'll see it appear in your channel list, visible only to you and the bot.

If Ollama is not running, the cog will be skipped gracefully:

```
Ollama not reachable at http://localhost:11434 — overwatch cog will not start
Overwatch cog failed to load — skipping
```
...
The overwatch dashboard is a separate FastAPI app:

```bash
# From the project root
OVERWATCH_DB_PATH=./overwatch.db uvicorn overwatch_dashboard.main:app --host 0.0.0.0 --port 8000
```
...
## 8. Available Commands (in `#overwatch-zone`)
...
1. **Every 15 minutes** (outside quiet hours), the bot pings you in `#overwatch-zone` asking for a progress update.
...
Set `OVERWATCH_ENABLED=false` in your `.env` and restart the bot. The cog won't load, the `#overwatch-zone` channel stays but goes silent. Delete the channel manually if you want it gone.
